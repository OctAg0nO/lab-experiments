...
| [14-durable-meta-agent](./14-durable-meta-agent.md) | 35 files + config + dapr resources + swarm | `DurableMetaAgent`, `DurableMetaConfig`, `GeneratedDurableAgent`, `wrap_module()`, `DaprFrontier`, `DaprLSEOptimizer`, `DaprAgentStack`, `AssessBatchSaturation`, `SwarmCoordinator`, `SwarmMetaAgent`, `SwarmTask`, `SwarmDiscovery`, `SwarmHeartbeat`, `SwarmInquiry`, `SwarmResponse` + all lab 13 modules | Same as lab 13 + `workflow_entry`, `DaprChatClient`, `DurableAgent`, `@message_router`, `call_agent()` |
