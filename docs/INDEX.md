...
├── 15_ray_sglang/          Distributed high-throughput: SGLang inference + Ray parallel agents
...
| [15-ray-sglang](./15-ray-sglang.md) | 8 new files + Lab 14 base | `MetaConfig` dataclass (validated), `ModuleExecutor` ABC, `InProcessExecutor`, `RayModuleExecutor` (bytes transport, weakref cache, order-preserving batch, circuit-breaker), `RayNotAvailableError`, `parallel_lse_evaluate()` (reuses Ray executor), `_pickle_module()` (bytes serializer), `_get_module_bytes()` (cached), `sglang.launch_server` scripts (4-bit AWQ + FP8 KV), `with_bridge` decorator | Same as lab 14 + `RayModuleExecutor.execute()`, `parallel_lse_evaluate()`, `_pickle_module()` |
| [15b-livekit-a2ui](./15b-livekit-a2ui.md) | 10 new files + Lab 15 base | `OctAg0nOAgent(Agent)` (llm_node streaming + thinking steps), `Qwen3TTS` (LiveKit plugin, 3 modes: CustomVoice/VoiceDesign/VoiceClone), `A2UIChannel` (v0.10 protocol, surfaces, theming), `A2UITransport` ABC (`LiveKitDataTransport`, `WebSocketTransport`), `AGUIHandler` (shared state, frontend tools, interrupt routing), `InterruptSystem` (human-in-the-loop confirm/input), `frontend_tools` (geolocation, file picker, clipboard), `ModelProfile` registry (4-role hybrid: Llama 4 Omni, DeepSeek-V4-Lite, Phi-4-Pro, NeMo-v3), `configure_all_models()`, `sglang_launch_commands()`, CopilotKit setup guide | Same as lab 15 + `llm_node` override, `a2ui_*()` tools, `AG-UI` events, `dspy.context(lm=...)` role routing |
