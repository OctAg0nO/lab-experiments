...
from ..ray.executor import InProcessExecutor, ModuleExecutor
...
@dataclass
class MetaConfig:
    """Configuration for MetaAgent construction.

    Centralizes all optional dependencies with sensible defaults.
    Follows the same pattern as DurableMetaConfig.
    Add new features as fields here instead of MetaAgent constructor params.
    """
    ...
    def __post_init__(self):
        if self.generator is None:
            raise ValueError("MetaConfig.generator is required")
...
class MetaAgent:
    ...
    Takes a single MetaConfig dataclass instead of 9 positional parameters.
    Add new features as fields on MetaConfig instead of constructor parameters.

    Legacy keyword-arg style (MetaAgent(llm=..., generator=...)) still works
    for backward compatibility with any code that hasn't been updated yet.
    """

    def __init__(self, config: MetaConfig | None = None, **kwargs):
        if config is not None:
            self._config = config
        else:
            self._config = MetaConfig(**kwargs)

        cfg = self._config
        self._llm = cfg.llm
        self._generator = cfg.generator
        self._tool_defs = cfg.tool_defs or []
        self.budget = cfg.budget or ResourceBudget()
        self.stack = cfg.stack or AgentStack()
        self.frontier: FrontierABC = cfg.frontier or InMemoryFrontier()
        self.lse = cfg.lse or LSEOptimizer()
        self._executor = cfg.executor or InProcessExecutor()
        self._consolidator = SkillConsolidator(cfg.skills_dir or "/tmp/skills")
    ...
    def run_stack_iter(self, task: str, max_iterations: int = 5):
        ...
        for iteration in range(max_iterations):
            ...
            try:
                ...
                prediction = self._executor.execute(module, task=direction.topic)
