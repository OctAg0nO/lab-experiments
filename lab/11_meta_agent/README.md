...
├── cli.py                               # Click CLI: generate, run, stack, distill
├── meta/
│   ├── agent_stack.py                   # AgentEntry + AgentStack (push/pop/query)
│   ├── agent_generator.py               # AnalyzeTask + GenerateSignature DSPy modules
│   └── meta_agent.py                    # MetaAgent orchestrator (LSE + Stack + Trace2Skill)
├── evolution/                           # Copied from 10: LSEOptimizer, SkillConsolidator
├── memory/                              # Copied from 10: InMemoryFrontier, NoopStore
...
Instead of fixed agents (Explorer/DeepReader/Synthesizer/Critic), the meta-agent:

1. **Analyzes** the user's task via `AnalyzeTask` DSPy signature
2. **Generates** new agent definitions using `GenerateSignature` DSPy module
3. **Pushes** them onto an `AgentStack` (name, role, goal, signature, tools)
4. **Selects** the best agent from the stack via `SelectNextAgent` DSPy module
5. **Runs** the generated agent against the task
6. **Evaluates** via LSEOptimizer (quality tracking per agent)
7. **Consolidates** patterns via SkillConsolidator (Trace2Skill)

## CLI Commands

```bash
# Generate agents for a task
uv run python -m lab.11_meta_agent --query "Your task" generate

# Full pipeline: generate → run → LSE → consolidate
uv run python -m lab.11_meta_agent --query "Your task" --iterations 5 run

# Inspect the agent stack
uv run python -m lab.11_meta_agent stack
